...
const categories = {
  MEN: [
    { name: "Baggey", img: "/image/baggeyImg.png", link: "/products/men/baggey" },
  ...
  WOMEN: [
    ...
    // { name: "Outerwear", img: "/image/womensImg.png", link: "/products/women/outerwear" },
  ],
  KIDS: [
         ...
         <div className="w-full">
            <div className="block  lg:hidden overflow-hidden">
             <AnimatePresence mode="wait">
               ...
               >
                 {categories[activeTab].map((cat) => (
                   <Link key={cat.name}  href={cat.link} className="flex-shrink-0 w-40 relative">
                     <Image
                       src={cat.img}
                       alt={cat.name}
                     ...
                     />
                     <div  className=" absolute text-white  bottom-2 left-5/12  text-[12px] drop-shadow-lg font-bold">{cat.name}</div>
                   </Link>
                 ))}
               </motion.div>
             </AnimatePresence>
